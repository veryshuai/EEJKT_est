...
if breakflag == 0
    ...
    %preallocate
    ind(:,:) = repmat([0,0,0,0,-1,-1,-1,-1,zeros(1,2),-1,-1,-1],mms,1); % holds state
        ...
        if scalar_temp ~= 0
            for k = lag:lag+scalar_temp-1
                ind(obin,:) = [cum_sp_p(j,k),...
                    ...
                    0,0,-1,-1,-1,-1,zeros(1,2),-1,-1,-1];
                [obin,max_mat_violation,vio] = step(obin,mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
            end
            lag = scalar_temp+1;
        end
        ind(obin,:) = [time,find(rand<cum_erg_pp,1,'first'),0,0,0,0,0,0,zeros(1,2),-1,-1,-1];
...
%resort and fill in
    ...
    ind(obin+1:end,:) = repmat([0,0,0,0,-1,-1,-1,-1,zeros(1,2),-1,-1,-1],scalar_temp,1);
...
obin_fix = zeros(S,1); % set 'first' index
