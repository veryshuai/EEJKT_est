...
%foreign
next = actual_f(3,2);
exp_inv_temp = expinv(rand,1/L_f);
gap = exp_inv_temp;
    while gap<burn+1
        ind(obin,1) = burn+1-gap;
        ind(obin,4) = next+1-2*(rand<.5*(1+(next-x_size-1)/x_size));
        next = ind(obin,4);
        [obin,max_mat_violation,vio] = step(obin,mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
        exp_inv_temp = expinv(rand,1/L_f);
...
next = actual_h(3,2);
g = 1;
exp_inv_temp = expinv(rand,1/L_h);
gap = exp_inv_temp;
    while gap<burn+1
        ind(obin,1) = burn+1-gap;
        ind(obin,3) = next+1-2*(rand<.5*(1+(next-x_size-1)/x_size));
        next = ind(obin,3);
        [obin,max_mat_violation,vio] = step(obin,mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
        exp_inv_temp = expinv(rand,1/L_h);
        gap = gap + exp_inv_temp;
...
%resort and fill in
    ...
    ind(obin+1:end,:) = repmat([0,0,0,0,-1,-1,-1,-1,zeros(1,2),-1,-1],size(ind(obin+1:end,:),1),1);
...
%get foreign matches
        ...
        for n = 1:size(deathind,1)
            ...
            for k = lag:deathind(n)
                exp_inv_temp = expinv(rand,1 / lambda_f(s_obs+1,m_obs+1,th_ind(j,1),min(s,net_size)+1,ind(k-1,2),ind(k-1,4)));
                ...
                if spell < gap
                    ...
                    while spell < gap && p < mult_match_max
                        ...
                        if rand<mu_f(j) %check for success
                            ...
                            exp_inv_temp = expinv(rand,1/delta);
                        ...
                        exp_inv_temp = expinv(rand,1/lambda_f(s_obs+1,m_obs+1,th_ind(j,1),min(s,net_size)+1,ind(k-1,2),ind(k-1,4))); %swing again
                        spell = exp_inv_temp;
...
%get home matches
        ...
        for n = 1:size(deathind,1)
            ...
            s=0;%success
            for k = lag:deathind(n)
                %display([k,n,lag,deathind(n),ind(k-1,2),ind(k-1,4),th_ind(1),th_ind(2),min(s,net_size),ind(k-1,2),ind(k-1,3)]);
                exp_inv_temp = expinv(rand,1/lambda_h(th_ind(j,1),th_ind(j,2),min(s,net_size)+1,ind(k-1,2),ind(k-1,3)));
                spell = exp_inv_temp;
...
for k = 1:obin_fix
    if ind(k,12) > 0 && ind(k,10) <= maxc %check that we still have room for more trajectories
        ...
        exp_inv_temp = expinv(rand,1/L_z); %time before next demand shock change 
        ...
        while cum_spell < ind(k,12) && p<=max_client_prod %until we reach exogenous separation time, keep the ds changes coming.
            ...
            exp_inv_temp = expinv(rand,1/L_z);
              ...
              if isempty(loop_ind) == 0 
                for o = 1:size(loop_ind,1)
                    ...
                    while tot_time < rel_time
                        exp_inv_temp = expinv(rand,1/L_b);
                    ...
                    while p<deathind(n) && (ind(p,1)-ind(k,1)<rel_time-1e-12) %note: I had to take a little bit off here to deal with rounding error
                        if dropped == 0 && sh(p-1,t) ~= 0; %sale is made
                            if c_val_h(ds(p-1,t),ind(p-1,2),ind(p-1,3))==0 %check for endogenous separation in the last period
                                sh(p,t) = 0; %kill any sales in current period  
                                ind(p,9) = ind(p,9)-1; %reduce current clients by one
                                dropped = 1;
                            end
                        elseif dropped == 1   
                            sh(p,t) = 0; %kill further sales if relationship is over
                            ind(p,9) = ind(p,9)-1; %reduce current clients by one
                        end
                        p = p+1;
                    end
