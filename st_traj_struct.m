function [ind,st,ds,sh,act,breakflag,deathmat] = st_traj_struct(th_ind,mu_f,mu_h,sp_p,lambda_f,lambda_h,c_val_h,c_val_f,mm,ri,rv,rs)
...
%get Phi's (self productivities)    
    ind{j}(obin(j),2) = find(rv(ri)<cum_erg_pp,1,'first');%initial productivity drawn out of ergodic distribution 
    lag = 1;
    time = 0;
    while time < TT
        time = time + expinv(rv(ri),1/d);
        ri = mod(ri,rs)+1;
        ...
        if death ~= 1
            for k = lag:death-1
                ind{j}(obin(j),:) = [cum_sp_p(j,k),...
                    ind{j}(obin(j)-1,2)+1-2*(rv(ri)<.5*(1+(ind{j}(obin(j)-1,2)-Phi_size-1)/Phi_size)),...
        ...
        ind{j}(obin(j),:) = [time,find(rv(ri)<cum_erg_pp,1,'first'),0,0,0,0,0,0,zeros(1,2),-1,-1];
        ri = mod(ri,rs)+1;
...
%foreign
next = actual_f(3,2);
gap = expinv(rv(ri),1/L_f);
ri = mod(ri,rs)+1;
    while gap<burn+1
        ind{j}(obin(j),1) = burn+1-gap;
        ind{j}(obin(j),4) = next+1-2*(rv(ri)<.5*(1+(next-x_size-1)/x_size));
        ri = mod(ri,rs)+1;
        next = ind{j}(obin(j),4);
        [obin(j),max_mat_violation,vio] = step(obin(j),mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
        gap = gap + expinv(rv(ri),1/L_f);
        ri = mod(ri,rs)+1;
    end
...
next = actual_h(3,2);
g = 1;
gap = expinv(rv(ri),1/L_h);
    ri = mod(ri,rs)+1;
    while gap<burn+1
        ind{j}(obin(j),1) = burn+1-gap;
        ind{j}(obin(j),3) = next+1-2*(rv(ri)<.5*(1+(next-x_size-1)/x_size));
        ri = mod(ri,rs)+1;
        next = ind{j}(obin(j),3);
        [obin(j),max_mat_violation,vio] = step(obin(j),mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
        gap = gap + expinv(rv(ri),1/L_h);
        ri = mod(ri,rs)+1;
    end
...
%get foreign matches
        ...
        for n = 1:size(deathind_ext,1)
            ...
            for k = lag:deathind_ext(n)
                ...
                spell = expinv(rv(ri),1/lambda_f{s_obs+1,m_obs+1,th_ind(j,1),min(s,net_size)+1}...
                ...
                if spell < gap             
                    ...
                    while spell < gap && p < mult_match_max
                        ...
                        m = m+1;                
                        if rv(ri)<mu_f(j) %check for success
                            ri = mod(ri,rs)+1;
                            s = s + 1;
                            if m<=n_size m_obs = m; s_obs = s; end
                            rel_time = min(expinv(rv(ri),1/delta),ind{j}(deathind_ext(n),1)-ind{j}(k-1,1)-cum_spell-1e-12); %exogenous match separation time
                            ri = mod(ri,rs)+1;
                            [obin(j),max_mat_violation,vio] = step(obin(j),mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
                        ...
                        spell = expinv(rv(ri),1/lambda_f{s_obs+1,m_obs+1,th_ind(j,1),min(s,net_size)+1}(ind{j}(k-1,2),ind{j}(k-1,4))); %swing again
                        ri = mod(ri,rs)+1;
...
%get home matches
        ...
        for n = 1:size(deathind_ext,1)
            ...
            for k = lag:deathind_ext(n)
                ...
                spell = expinv(rv(ri),1/lambda_h{th_ind(j,1),th_ind(j,2),min(s,net_size)+1}(ind{j}(k-1,2),ind{j}(k-1,3)));
                ri = mod(ri,rs)+1;
                ...
                if spell < gap             
                    ...
                    while spell < gap && p < mult_match_max
                        ...
                        s = s + 1;
                            ri = mod(ri,rs)+1;
                            [obin(j),max_mat_violation,vio] = step(obin(j),mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
                            rel_time = min(expinv(rv(ri),1/delta),ind{j}(deathind_ext(n),1)-ind{j}(k-1,1)-cum_spell-1e-12);
                            ri = mod(ri,rs)+1;
                            [obin(j),max_mat_violation,vio] = step(obin(j),mms,max_mat_violation); if vio == 1 breakflag = 1; break; end
...
%recorded in the last section.
    ...
    for k = 1:obin_fix
        if ind{j}(k,12) > 0 && ind{j}(k,10) <= maxc %check that we still have room for more trajectories
            ...
            [ds{j}(k,slot),~] = find(rv(ri)<cum_erg_pz,1,'first'); %get initial demand shock from ergodic dist.
            ri = mod(ri,rs)+1;
            cum_spell = expinv(rv(ri),1/L_z); %time before next demand shock change 
            ri = mod(ri,rs)+1;
            ...
            while cum_spell < ind{j}(k,12) && p<=max_client_prod %until we reach exogenous separation time, keep the ds changes coming.
                ...
                if p == 1
                    new_st = ds{j}(k,slot)+1-2*(rv(ri)<.5*(1+(ds{j}(k,slot)-z_size-1)/z_size)); 
                    ri = mod(ri,rs)+1;
                else
                    new_st = ds{j}(obin(j)-1,slot)+1-2*(rv(ri)<.5*(1+(ds{j}(obin(j)-1,slot)-z_size-1)/z_size));
                    ri = mod(ri,rs)+1;
                end
                ds{j}(obin(j),slot) =  new_st; %record the new state in the ds matrix
                cum_spell = cum_spell + expinv(rv(ri),1/L_z);
                ri = mod(ri,rs)+1;
            end
        end
    end
    ...
    g=0; %index for random vector used to calculate client productivities    
     for k = 1:obin_fix
        if ind{j}(k,11) > 0 && ind{j}(k,9) <= maxc
            ...
            ds{j}(k,end-1) = slot(1,1);
            ds{j}(k,slot) = find(rv(ri)<cum_erg_pz,1,'first'); %get initial prod from ergodic dist.
            ri = mod(ri,rs)+1;
            cum_spell = expinv(rv(ri),1/L_z);  
            ri = mod(ri,rs)+1;
            p = 0;%this keeps track of number of demand shock changes
              ...
              if isempty(loop_ind) == 0 
                for k = loop_ind'
                    ...
                    while tot_time < rel_time
                        next_ship_time = expinv(rv(ri),1/L_b);
                        ri = mod(ri,rs)+1;
...
display(['At end of st_traj, ri is', num2str(ri), 'and the final random draw is ', num2str(rv(ri)),'.'])
