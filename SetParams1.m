...
%% technology parameters
    ...
    mm.gam       = 0;         % Network effect parameter
    
%% Theta distributions

    mm.ah        = ah;         % Beta function, home (theta1) success parameter
    mm.bh        = bh;         % Beta function, home (theta1) failure parameter
    mm.af        = ah;         % Beta function, foreign (theta2) success parameter (assume same as home)
    mm.bf        = bh;         % Beta function, foreign (theta2) success parameter (assume same as home)
    mm.gamma1    = mm.ah/(mm.ah+mm.bh); % true mean home (theta1) distribution
    mm.gamma2    = mm.gamma1;     % true mean of foreign (theta2) distribution
...
%% Discretization of state-space
    ...
    mm.n_size        = 10;     % Maximum number of informative signals per firm (WAS 20)
    mm.cost_n        = 20;     % Number of clients for which cost decreases.
    mm.z_size        = 15;     % Number of discretized buyer states (2*n+1) 
    mm.phi_size      = 15;     % number of different discretized profit shocks (2*n+1)
    mm.x_size        = 7;     % Number of different discretized macro shocks; same for home and foreign (2*n+1)  
    mm.lambda_size   = 40;     % Number of possible effort levels
    mm.theta_size    = 51;     % Number of possible market potential values
    mm.dim0          = 3;      % Number of possible theta0 values (common to both markets)
    mm.dim1          = 50;      % Number of possible theta1 values (specific to home market); 
    mm.dim2          = 50;      % Number pf possible theta2 values (specific to foreign market); 
...
%% Solution parameters
    mm.v_tolerance   = 5e-3;  % convergence tolerance, value function iterations (WAS .005)
    mm.pi_tolerance  = 1e-7;  % convergence tolerance, profit function (WAS .001)
    mm.T             = 50;     % horizon for calculating profit function
    mm.S             = 2000;      % number of potential exporting firms to simulate (WAS 2000)
    mm.burn          = 10;        %number of burn-in periods
...
%     save('exog_est/exog.mat','L_h','D_h','Q_h','X_h','L_f','D_f','Q_f','X_f','actual_f','actual_h');
    ...
    % create intensity matrices for productivity parameters
        D_z = .5; %delta, size of jump states
        [Q_z,Z] = makeq(L_z,D_z,mm.z_size);
        erg_pz = make_erg(L_z,D_z,Z); 
        
        D_p = .5; %delta, size of jump states
    ...
    mm.D_z          = .5;       %size of jump in other firms productivity
    mm.Q_z          = Q_z;      %intensity matrix for seller productivities 
    mm.Q_z_d        = Q_z_d;    %with zeros on the diagonal
    mm.erg_pz       = erg_pz;   %ergodic distribution of seller productivities
